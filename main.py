...
from flask import Flask, jsonify                                from colorama import Fore, init

init(autoreset=True)
requests.packages.urllib3.disable_warnings()                    
# ─── OPTIONAL PLAYWRIGHT ─────────────────────────────────────────────────────                                                 JS_LOCAL_AVAILABLE = False                                      try:
    from playwright.sync_api import sync_playwright                 JS_LOCAL_AVAILABLE = True
except Exception:                                                   JS_LOCAL_AVAILABLE = False                                                                                                  # ─── GLOBALS ─────────────────────────────────────────────────────────────────                                                 visited = set()                                                 findings = []                                                                                                                   BASE_HEADERS = {"User-Agent": "kArmas-ZAP-FULL/1.2"}            SESSION_HEADERS = BASE_HEADERS.copy()                                                                                           # ─── HEADER SAFETY ───────────────────────────────────────────────────────────                                                 def sanitize_headers(headers):                                      clean = {}                                                      for k, v in headers.items():
        try:                                                                v.encode("latin-1")
            clean[k] = v                                                except UnicodeEncodeError:
            clean[k] = v.encode("utf-8", "ignore").decode("latin-1", "ignore")                                                      return clean                                                
# ─── DATABASE ────────────────────────────────────────────────────────────────
db = sqlite3.connect("karmas_zap_full.db", check_same_thread=False)
cur = db.cursor()                                               
cur.execute("""                                                 CREATE TABLE IF NOT EXISTS findings (                               type TEXT, target TEXT, severity TEXT, details TEXT         )
""")                                                            cur.execute("""
CREATE TABLE IF NOT EXISTS traffic (                                ts INTEGER, method TEXT, url TEXT, status INTEGER,              req_headers TEXT, resp_headers TEXT, resp_size INTEGER
)                                                               """)
db.commit()
...
def store_traffic(method, url, status, req_h, resp_h, size):
    ...
    db.commit()                                                 
# ─── HTTP WITH LOGGING ───────────────────────────────────────────────────────                                                 def http_get(url):
    r = requests.get(url, headers=SESSION_HEADERS, timeout=12, verify=False)                                                        store_traffic(
        "GET", url, r.status_code,                                      dict(SESSION_HEADERS), dict(r.headers), len(r.content)
    )
    return r
                                                                # ─── HTTP CRAWLER ────────────────────────────────────────────────────────────                                                 def http_crawl(url, base):
    if url in visited:                                                  return
    visited.add(url)                                                                                                                try:
        r = http_get(url)
    except Exception:                                                   return                                                                                                                      # Use html.parser — lxml not needed and often missing on Termux                                                                 soup = BeautifulSoup(r.text, "html.parser")
                                                                    parsed = urlparse(url)
    if parsed.query:                                                    store_finding("Params", url, "INFO", ",".join(parse_qs(parsed.query).keys()))                                           
    for tag in soup.find_all(["a", "form"], href=True):                 link = urljoin(base, tag["href"])
        if base in link and link not in visited:                            http_crawl(link, base)

# ─── JS CRAWLER (LOCAL OR REMOTE) ────────────────────────────────────────────                                                 def js_crawl(start_url, base, remote_ws=None):                      try:                                                                with sync_playwright() as p:                                        if remote_ws:
                print(Fore.CYAN + "[*] Connecting to REMOTE headless browser")
                browser = p.chromium.connect_over_cdp(remote_ws)            else:
                if not JS_LOCAL_AVAILABLE:
                    raise RuntimeError("Local Playwright not available")
                print(Fore.CYAN + "[*] Using LOCAL headless browser")
                browser = p.chromium.launch(headless=True)                                                                                  context = browser.new_context(extra_http_headers=SESSION_HEADERS)
...
def headers_check(url):
    ...
    for c in r.cookies:
        ...
        if not c.has_nonstandard_attr("HttpOnly"):                          issues.append("HttpOnly")
        if issues:
...
def verify_auth(url):                                               r = http_get(url)
    if r.status_code in (401, 403):
        store_finding("Auth", url, "HIGH", "Auth failed or expired")                                                                else:
...
def api_traffic():
    rows = cur.execute(
        "SELECT ts,method,url,status,req_headers,resp_headers,resp_size FROM traffic"                                               ).fetchall()
    out = []
    for row in rows:
        out.append({
            "ts": row[0],                                                   "method": row[1],
            "url": row[2],
            "status": row[3],
...
def build_headers(args):                                            h = BASE_HEADERS.copy()
    if args.cookie:
        h["Cookie"] = args.cookie
    if args.auth:
        if ":" in args.auth:
            k, v = args.auth.split(":", 1)                                  h[k.strip()] = v.strip()
...
def main():
    ...
    print(Fore.CYAN + f"[*] Target: {target}")
                                                                    if args.js:
        js_crawl(target, base, remote_ws=args.remote_ws)            else:
